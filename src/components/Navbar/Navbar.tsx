"use client";

import {
  ChevronDownIcon,
  ChevronLeftIcon,
  Cog8ToothIcon,
} from "../../../node_modules/@heroicons/react/24/outline";
import React, { useState } from "react";
import { navbarData} from "./NavBarData";
import { usePathname, useRouter } from "next/navigation";
import Image from "next/image";
import Link from "next/link";
import { NavbarItem } from "./components/NavbarItem";


interface NavbarProps {
  title: string;
  subtitle?: string;
  subtitleChildren?: React.ReactNode;
  user?: any;
}

export const NavBar = ({ title, subtitle, subtitleChildren, user }: NavbarProps) => {
  const pathName = usePathname() ?? "";
  const router = useRouter();
  const isSecondaryPage = pathName.split("/").length > 2;
  const [isOpen, setisOpen] = useState(false);

  return (
    <div className="flex relative w-full justify-between items-center px-10 pt-10">
      <div className="flex flex-col ">
        <div className="flex items-center gap-5">
          {isSecondaryPage && (
            <div
              className="bg-pale-blue rounded-md p-1 hover:cursor-pointer hover:bg-blue-300"
              onClick={() => {
                router.back();
              }}
            >
              <ChevronLeftIcon className="w-5 h-5 text-medium-blue" />
            </div>
          )}
          <h1 className="text-2xl font-bold text-dark-blue lg:max-w-xs xl:max-w-lg">
            {title}
          </h1>
        </div>
        {subtitle && (
          <h2 className="text-sm font-light text-subtitle-blue">{subtitle}</h2>
        )}
        {subtitleChildren}
      </div>
      <div className="flex w-1/2 justify-end gap-[6px] rounded-2xl">
        
        <div className="flex relative items-center w-full h-[69px] bg-white shadow-lg rounded-2xl justify-between px-5">
          <div className="flex gap-6">
            <Image
              src="data:image/jpeg;base64,"
              alt="logo"
              width={40}
              height={40}
            />
            <div className="flex flex-col">
              <h1 className="text-dark-blue text-base font-bold">
               {user.name}
              </h1>
              <h2 className="text-dark-blue text-xs">{user.user_type}</h2>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <Cog8ToothIcon className="w-7 h-7 text-medium-blue" />
            <ChevronDownIcon
              className={`text-medium-blue duration-100 ${
                isOpen ? "rotate-180" : "rotate-0"
              }`}
              height={18}
              onClick={() => {
                setisOpen(!isOpen);
              }}
            />
          </div>
          <div
            className={`z-10 absolute w-full flex flex-col gap-2 py-6 bg-white shadow-lg px-5 left-0 top-[60px] rounded-b-2xl duration-300 ${
              isOpen ? "block" : "hidden"
            }`}
          >
            {navbarData.map((item, index) => (
              <div
                className={`${
                  navbarData.length - 1 != index &&
                  "border-b-[0.5px] border-dark-gray"
                }`}
                key={index}
              >
                <NavbarItem
                  key={index}
                  title={item.title}
                  path={item.path}
                  icon={item.icon}
                />
              </div>
            ))}
          </div>
        </div>
      </div>
    </div>
  );
};
